...
function CustomDrawerContent(props) {
  ...
  const [updatebankdetils, setUpdateBankDetails] = useState('');
    ...
    axios
      ...
      .then(response => {
        if (response.data.status === 1) {
          ...
          setloading(false)
          try {
            setUpdateBankDetails(response?.data?.data?.updated_bank_details)
            setUserdata({
  ...
  const UpdateBank = async () => {
    ...
    formData.append('aadhar_no', Userdata?.aadhar_no)
    formData.append('branch', Userdata?.branch)
    formData.append('account_holder_name', Userdata?.account_number)

    fetch(`${apiUrl}/api/update_employee_bank_details`, {
      method: 'POST',
      headers: {
        'Token': token,
        Accept: 'application/json',
        ...
        <View style={{ padding: 10 }}>
          <View style={{ marginVertical: 10 }}>
            ...
            <TextInput
              ...
              placeholderTextColor={theme == 'dark' ? '#000' : '#000'}
              editable={updatebankdetils == 0 ? true : false}
              selectTextOnFocus={false}
              value={Userdata.name}
          ...
          <View style={{ marginVertical: 10 }}>
            ...
            <TextInput
              ...
              onChangeText={(value) => handleInputChange('aadhar_no', value)}
            />
          </View>
          {
            updatebankdetils == 0 ?
              <TouchableOpacity onPress={() => UpdateBank()} style={{ flex: 1, backgroundColor: "blue", padding: 10, borderRadius: 5, marginBottom: 10, flexDirection: "row", marginHorizontal: responsiveWidth(25), alignSelf: "center", alignItems: "center", alignContent: "center" }}>
                {loading ? <ActivityIndicator size="small" color="#388aeb" /> : <Text style={{ textAlign: "center", color: "#fff" }}>Submit</Text>}
              </TouchableOpacity>
              :
              null
          }
