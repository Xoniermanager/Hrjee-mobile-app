...
const Processing = () => {
  ...
  const [offRefresh, setOffRefresh] = useState(true)
  ...
  const get_employee_detail = async () => {
    ...
    axios
      .get(`${apiUrl}/SecondPhaseApi/processing_tasks`, config)
      .then(response => {
        setloading(false);
        setOffRefresh(false)
        if (response?.data?.status == 200) {
          setUserdata(response?.data?.data?.data);
          setNextFetch(response?.data?.data?.next)
        }
      })
      .catch(error => {
        setOffRefresh(false)
  ...
  const fetchMore = async () => {
    ...
    if (nextFetch != null) {
      ...
      console.log(`${apiUrl}/SecondPhaseApi/processing_tasks?page=${nextPage}`)
      axios
        .get(`${apiUrl}/SecondPhaseApi/processing_tasks?page=${nextPage}`, config)
        .then(response => {
          setloading(false);
          if (response?.data?.status == 200) {
            console.log(response?.data.data.next, 'hh')
            setNextFetch(response?.data.data.next)
            setIsLoader(response?.data?.data?.data);
            setCurrentPage(nextPage);
            setUserdata(prevData => [...prevData, ...response?.data.data.data]);
  ...
  const tast_status_update = async item => {
    setloading(true);
    setShowAddress(showAddress + 1);
    console.log("taskmaxradious--------------->", taskmaxradious)
    ...
    if (coordinates?.lat == undefined || coordinates?.lng == undefined) {
      ...
      Popup.show({
        ...
        title: 'Warning',
        button: true,
        textBody: 'Visit address found invalid',
        buttonText: 'Ok',
        callback: () => [Popup.hide()],
      });
      setloading(false);
      ...
      else if (taskmaxradious > 0) {
        if (taskmaxradious >= dis) {
          if (Platform.OS === 'android') {
            try {
              const granted = await PermissionsAndroid.request(
                PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
    ...
    <>
      ...
      <View style={styles.container}>
        <Root>
          ...
          <FlatList
            data={(filterData ? filterData : Userdata)} // Limit to first 5 items
            onEndReached={() => fetchMore()}
            onEndReachedThreshold={0.1}
            ListFooterComponent={() =>
              isLoader && isLoader.length == 0 ? null : <Reload />
            }
            refreshControl={
              <RefreshControl
                refreshing={offRefresh}
                onRefresh={handleRefresh}
              />
            }
            renderItem={({ item, index }) => (
              <>
                <View key={index} activeOpacity={0.2} style={styles.maincard}>
                  <View
                    style={{
                      flexDirection: 'row',
                      justifyContent: 'space-between',
