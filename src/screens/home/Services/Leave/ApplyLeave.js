...
const ApplyLeave = ({ navigation }) => {
  ...
  const [logs, setLogs] = useState({
    ...
    exit_entry_visa_reqd: '',
    accept_leave_policy: '',
    current_approver_eno: ''
  })
  ...
  callApplyLeaveAPI = async () => {
    ...
    setLogs({
      ...
      leavetype: value,
      leave_wfstage_id: 11,
      leave_start_date: `${startDate.getFullYear()}-${startDate.getMonth() + 1}-${startDate.getDate()}`,
      leave_end_date: `${endDate.getFullYear()}-${endDate.getMonth() + 1}-${endDate.getDate()}`,
      morning_evening: halfDay,
      notes: comment,
      guaranter_id: user.employee_number,
      emergency_contact_name: form.name,
      emergency_contact_phone: form.phone,
      emergency_contact_address: form.address,
      exit_entry_visa_reqd: 1,
      accept_leave_policy: isCheck ? 1 : 0,
        ...
        if (response.data.status == 1) {
          try {
            Popup.show({
              ...
              callback: () => [
                Popup.hide(),
                LogsMaintane(response.data.message),
                navigation.navigate('Leave Applied List'),
              ],
            });
          } catch (e) {
            setDisabledBtn(false);
          ...
          Popup.show({
            ...
            textBody: response.data.message,
            buttonText: 'Ok',
            callback: () => [Popup.hide(), LogsMaintane(response.data.message)],
          });
        }
      })
  ...
  const apply_leave = async () => {
    ...
    if (startDate.valueOf() <= endDate.valueOf() && halfDay === '0') {
      callApplyLeaveAPI();
    } else if (halfDay === 'A' || halfDay === 'P') {
      callApplyLeaveAPI();
    } else {
      setDisabledBtn(false);
      Popup.show({
        ...
        buttonText: 'Ok',
        callback: () => [Popup.hide(), LogsMaintane('End date greater then start date')],
      });
    }
  };
  ...
  const LogsMaintane = async (mes) => {
    ...
    const payload = {
      ...
      payload: {
        userid: logs?.userid,
        leave_balance: logs?.leave_balance,
        region_name: logs?.region_name,
        leavetype: logs?.leavetype,
        login_type: logs?.login_type,
        leave_wfstage_id: logs?.leave_wfstage_id,
        leave_start_date: logs?.leave_start_date,
        leave_end_date: logs?.leave_end_date,
        morning_evening: logs?.morning_evening,
        notes: logs?.notes,
        guaranter_id: logs?.guaranter_id,
        emergency_contact_name: logs?.emergency_contact_name,
        emergency_contact_phone: logs?.emergency_contact_phone,
        emergency_contact_address: logs?.emergency_contact_address,
        exit_entry_visa_reqd: logs?.exit_entry_visa_reqd,
        accept_leave_policy: logs?.accept_leave_policy,
        current_approver_eno: logs?.current_approver_eno
      }
    };
    ...
    if (choosedata == 'Self') {
      ...
      if (value == null) {
        ...
        return Popup.show({
          ...
          buttonText: 'Ok',
          callback: () => [Popup.hide(), LogsMaintane('please select leave type')],
        });
      }
      ...
      if (name == null) {
        ...
        return Popup.show({
          ...
          buttonText: 'Ok',
          callback: () => [Popup.hide(), LogsMaintane('please enter name')],
        });
      }
      if (phone == null) {
        setDisabledBtn(false);
        return Popup.show({
          ...
          buttonText: 'Ok',
          callback: () => [Popup.hide(), LogsMaintane('please enter mobile')],
        });
      }
      if (address == null) {
        setDisabledBtn(false);
        return Popup.show({
          ...
          buttonText: 'Ok',
          callback: () => [Popup.hide(), LogsMaintane('please enter address')],
        });
      }
    }
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e3eefb', padding: 18 }}>
      <Root>
        <ScrollView style={{ marginHorizontal: 5 }}>
          ...
          <View style={styles.input_top_margin}>
            ...
            <Text
              style={[
                { marginLeft: 10, marginBottom: 10, marginTop: 5 },
                { color: Themes == 'dark' ? '#000' : '#000' },
              ]}>
              {leaveBalance != 0 ? leaveBalance : 0}
            </Text>
            <View
              style={{
                borderWidth: 0.5,
          ...
          {halfDay === '0' && (
            <View style={styles.input_top_margin}>
              ...
              <TouchableOpacity
                ...
                <Text style={{ color: '#000' }}>{endDate.toISOString().substring(0, 10)}</Text>
                <AntDesign name="calendar" size={20} style={styles.radio_icon} color="#0321a4" />
              </TouchableOpacity>
              {endopen && (
                <DatePicker
                  textColor="#000000"
